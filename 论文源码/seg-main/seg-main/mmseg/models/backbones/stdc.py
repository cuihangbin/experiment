...
class STDCNet(BaseModule):
    ...
    def __init__(self,
        ...
        self.conv0 = nn.Conv2d(3, 256, kernel_size=1, padding=0)  # 把第一个channel的数值：3升到32，最后再变回3（考虑到变量in_channels=3）
        self.conv00 = nn.Conv2d(256, 3, kernel_size=1, padding=0)  # 32变回3，改进模块结束
        self.conv1 = nn.Conv2d(256, 64, kernel_size=1, padding=0)  # 卷积模块1的x1
        self.conv11 = nn.Conv2d(64, 64, kernel_size=1, padding=0)  # 卷积模块1的x1通道之后，第一个卷积1*1
        self.conv12 = nn.Conv2d(64, 64, kernel_size=3, padding=1)  # 卷积模块1的x1通道之后，第二个卷积3*3
        self.conv13 = nn.Conv2d(64, 64, kernel_size=5, padding=2)  # 卷积模块1的x1通道之后，第三个卷积5*5
        self.conv2 = nn.Conv2d(256, 128, kernel_size=2, padding=0)  # 卷积模块1的x2
        self.conv31 = nn.Conv2d(256, 64, kernel_size=1,
                               padding=0)  # 卷积模块1的x3，和self.conv1一样，只不过这样写清清晰点self.conv31 = nn.Conv2d(8, 2, kernel_size=1, padding=0)  # 卷积模块1的x3通道之后，第一个卷积1*1(1/16)
        self.conv32 = nn.Conv2d(16, 16, kernel_size=3, padding=1)  # 卷积模块1的x3通道之后，第二个卷积3*3(1/16)
        self.conv33 = nn.Conv2d(16, 64, kernel_size=1, padding=0)  # 卷积模块1的x3通道之后，第三个卷积1*1(1/4)
    ...
    '''def forward(self,x):
        ...
        # x1
        x1 = self.conv1(x)
       # x1 = torch.sigmoid(torch.add(torch.add(self.conv11(x1), self.conv12(x1)), self.conv13(x1)))
        # x2
        x2 = self.conv2(x)
        # x3
        x3 = self.conv33(self.conv32(self.conv31(self.conv31(x))))
        # 卷积模块1输出
        x = self.conv00(torch.cat([torch.cat([x1, x2], 1), x3], 1))
        return x
'''
@BACKBONES.register_module()
